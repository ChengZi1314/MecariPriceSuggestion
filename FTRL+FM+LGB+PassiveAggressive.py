...
def main():
    ...
    model = FTRL(alpha=0.01, beta=0.1, L1=0.00001, L2=1.0, D=sparse_merge.shape[1], iters=30, inv_link="identity",
                 threads=1)
    del X;
    gc.collect()
    ...
    w = (0.10, 0.11, 0.23, 0.56)

    preds = preds * w[0] + predsF * w[1] + predsL * w[2] + predsFM * w[3]

    submission['price'] = np.expm1(preds)
    submission.to_csv("sub ftrl_fm_lgb_PAR.csv", index=False)
