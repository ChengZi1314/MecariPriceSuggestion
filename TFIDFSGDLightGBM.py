...
def cutting(dataset):
    ...
    pop_category = dataset['subcat_0'].value_counts().loc[lambda x: x.index != 'missing'].index[:NUM_BRANDS]
    ...
    dataset.loc[~dataset['subcat_0'].isin(pop_category), 'subcat_0'] = 'missing'
...
def main():
    ...
    train = pd.read_table(r'D:\MercariPriceSuggestion\train.tsv', engine='c')
    test = pd.read_table(r'D:\MercariPriceSuggestion\test.tsv', engine='c')
    ...
    merge['subcat_0'], merge['subcat_1'], merge['subcat_2'] = \
    ...
    cv = CountVectorizer()
    X_category1 = cv.fit_transform(merge['subcat_0'])
